...
const getContests = async (req, res) => {
  ...
  `${API_URL}?username=${USERNAME}&api_key=${API_KEY}&start__gte=${startTimestamp}&end__lte=${endTimestamp}&resource__in=codeforces.com,leetcode.com,codechef.com&orderby=start`
);

    // Check if the API request was successful
if (!response.ok) {
  const text = await response.text();
  console.log("CLIST Response:", text);
