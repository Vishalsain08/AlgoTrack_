...
export default function ContestList() {
  ...
  const [selectedReminderContest, setSelectedReminderContest] = useState(null);
  ...
  const openReminderModal = (contest) => {
    setSelectedReminderContest(contest);
    setIsReminderModalOpen(true);
  };

  const closeReminderModal = () => {
    setSelectedReminderContest(null);
    setIsReminderModalOpen(false);
  };

  const handleReminderSave = (timeBefore) => {
    if (!selectedReminderContest) return;

    toggleReminder(
      selectedReminderContest.id,
      selectedReminderContest.host
        .replace(".com", "")
        .replace(/^\w/, (c) => c.toUpperCase()),
      "email",
      timeBefore,
      selectedReminderContest.start
    );
  };
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto mt-4 p-2 sm:p-4">
                  ...
                  >
                    ...
                    {token && (
                      <td className="p-4 flex justify-center items-center gap-4">
                        <button
                          onClick={() => {
                            const platform = contest.host
                              .replace(".com", "")
                              .replace(/^\w/, (c) => c.toUpperCase());

                            if (isReminderSet(contest.id, platform)) {
                              toggleReminder(
                                contest.id,
                                platform,
                                "email",
      ...
      <ReminderModal
        ...
        contest={selectedReminderContest}
      />
    </div>
  );
}